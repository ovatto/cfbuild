...
const addApiLambdaPermissions = template => {
  ...
  const apiResources = findResources(template, 'AWS::ApiGateway::RestApi')
  Object.keys(apiResources).forEach((logicalName) => {
    const api = apiResources[logicalName]
    const paths = getAttribute(api, 'Properties.Body.paths')
    if(paths) {
      const arns = new Set()
      ...
      Array.from(arns).sort().forEach((arn, index) => {
        ...
        template.Resources[`${logicalName}LambdaPermission${index+1}`] = {
          ...
          Properties: {
            ...
            SourceArn: {
              'Fn::Sub': 'arn:aws:execute-api:${AWS::Region}:${AWS::AccountId}:${'+logicalName+'}/*/*/*'
            }
          }
        }
      })
...
const expandApis = template => {
  ...
  const apiResources = findResources(template, 'Wibe::RestApi')
  Object.keys(apiResources).forEach((logicalName) => {
    const api = apiResources[logicalName]
