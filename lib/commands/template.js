...
const BUILD_SCHEMA = jsyaml.DEFAULT_SCHEMA.extend([includeYamlType])
...
const isLambdaIntegrationType = type => (['aws', 'aws_proxy'].indexOf(type) !== -1)
    ...
    if (paths) {
      ...
      Object.keys(paths).map(pathName => paths[pathName]).forEach(path => {
        Object.keys(path).map(methodName => path[methodName]).forEach((method) => {
          ...
          if (integration && isLambdaIntegrationType(integration.type) && integration.uri) {
